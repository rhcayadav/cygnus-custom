...
public class CygnusApplication extends Application {
    ...
    public synchronized void handleConfigurationEvent(MaterializedConfiguration conf) {
        ...
        // stop YAFS thread to prevent shutdown.
        boolean restart = false;
        if (mYafs != null) {
            mYafs.requestStop();
            try {
                mYafs.join(YAFS_CHECKING_INTERVAL*2);
            } catch (InterruptedException ex) {
	        LOGGER.warn("can't join YAFS but continue...");
            }
            restart = true;
        }
        ...
        public void run() {
            ...
            while (mStopRequest == false) {
                ...
                try {
                    ...
                    if (mStopRequest) {
                        LOGGER.info("YAFS receive STOP request.");
                        return;
                    } else {
                        LOGGER.error("YAFS catch Unknown InnterruptedException. Exiting Cygnus.");
                        System.exit(-1);
                    }
                }
            } // while
